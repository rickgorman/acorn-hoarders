...
class Squirrel extends Occupant {
  constructor(cell, brain) {
    ...
    this.memory.forEach((el, idx) => {
      this.memory[idx] = Math.floor(Math.random() * 255);
    });
  }
  ...
  senseWorld() {
    ...
    for(let i=0; i < this.memory.length; i++) {
      this.inputs.push(this.memory[i] / 255);
    }
  ...
  _optionallySetMemoryCell(address, signal) {
    if (signal < 0.1) {
      // no change
    } else {
      this.memory[address] = Math.floor((signal - 0.1) * 255);
    }
  }
